      ...
      <q-card-section>
        <div class="text-h6 text-grey-8">
          ...
          <div class="float-right">
            <q-btn :label="$t('add')" :to="{name:'stateAdd'}" class="text-capitalize q-mr-md" icon="add"
                   color="primary"/>
          ...
          <template #body-cell-action="props">
            <q-td :props="props">
              <q-btn
                ...
                flat
                dense
                :to="{name:'stateEdit',params: {id:props.row.id}}"
              />
              <q-btn
                color="red"
                icon-right="delete"
...
export default defineComponent({
  ...
  methods: {
    ...mapActions('state', ['fetchStates', 'deleteState']),
    request(data = {}) {
      this.fetchStates(data).then((res) => {
    ...
    onDelete(item) {
      const index = this.states.indexOf(item.row);
      if (confirm(this.$t('sureToDelete'))) {
        this.states.splice(index, 1);
        this.deleteState({id: item.row.id});
      }
    },
    onBulkDelete() {
      if (confirm(this.$t('sureToDelete'))) {
        ...
        ids.forEach((x, i) => {
          let index = this.states.findIndex(function (u) {
            return u.id === x;
          });
          if (index !== -1) {
            this.states.splice(index, 1);
          }
          this.deleteState({id: x});
        });
      }
